...
if [ -f /etc/mSysmon/sys-auth.log ]; then
  token_file=($(cat /etc/mSysmon/sys-auth.log))
else
...
if [ -e /etc/mSysmon/pe-data.log ]; then
  data=($(cat /etc/mSysmon/pe-data.log))
...
echo "$time $cpu $io $idle $rx $tx" >/etc/mSysmon/pe-data.log
...
for di in $(lsblk -dnpl -o NAME | grep -v '^/dev/sr')
do
